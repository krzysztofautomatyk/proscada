<script lang="ts">
  ...
  const alarmsSuspended = $derived(
    readBoolean(config, "alarmsSuspended", false) ||
      groupAlarms.some((alarm) => alarm.evaluationSuspended),
  );
...
<button class="indicator {worst?.priority ?? 'normal'} {rollupState}" class:stale={alarmsSuspended} disabled={design || !!parsed.error} onclick={navigate} aria-label={`Alarm indicator for ${group}: ${alarmsSuspended ? "evaluation suspended, last known states" : `${active.length} alarms`}`}>
  {#if alarmsSuspended}
    <span class="icon" aria-hidden="true">▲</span><span class="copy"><b>ALARMS STALE</b><small>Last known states</small></span>
...
<style>
  ...
  .indicator.stale { border:2px solid #991b1b; background:repeating-linear-gradient(135deg,#fef2f2,#fef2f2 5px,#fee2e2 5px,#fee2e2 10px); color:#7f1d1d; }
</style>
