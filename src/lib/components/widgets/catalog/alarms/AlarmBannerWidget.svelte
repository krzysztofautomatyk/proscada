<script lang="ts">
  ...
  const alarmsSuspended = $derived(
    readBoolean(configOf(widget), "alarmsSuspended", false) ||
      parsed.alarms.some((alarm) => alarm.evaluationSuspended),
  );
...
<section class="banner" class:normal={!worst} class:critical={worst?.priority === "critical"} class:high={worst?.priority === "high"} aria-label="Alarm banner">
  {#if alarmsSuspended}
    <span class="icon" aria-hidden="true">▲</span><div class="copy"><b>ALARMS STALE</b><strong>Evaluation suspended</strong><small>Displayed states are last known, not live</small></div>
