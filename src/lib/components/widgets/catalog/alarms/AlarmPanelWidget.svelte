...
<section class="panel" aria-label="Alarm panel">
  ...
  {:else}
    ...
    {#if ackNotice}<p class="ack-notice" role="status">{ackNotice}</p>{/if}
  {/if}
</section>
...
<style>
  ...
  .shelved-overlay { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:repeating-linear-gradient(135deg,rgba(71,85,105,.14),rgba(71,85,105,.14) 7px,rgba(255,255,255,.25) 7px,rgba(255,255,255,.25) 14px); color:#334155; font-weight:900; letter-spacing:.08em; pointer-events:none; } .no-match,.ack-notice { margin:7px; color:#64748b; text-align:center; } .ack-notice { color:#166534; font-weight:700; }
</style>
